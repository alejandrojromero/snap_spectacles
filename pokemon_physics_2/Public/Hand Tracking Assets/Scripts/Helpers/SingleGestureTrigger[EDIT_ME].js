...
function initialize() {
    ...
    Physics.getRootWorldSettings().gravity = new vec3(0,-400,0);    
    if (!script.hintText) {
...
script.ball.getComponent("Physics.BodyComponent").onCollisionEnter.add(function (e) {
    var collision = e.collision;
    print("CollisionEnter(" + collision.id + "): ---> " + collision.collider.getSceneObject().name);

//    if(collision.collider.getSceneObject().name == "Floor"){
//            script.ball.getComponent("Physics.BodyComponent").clearMotion();
//            
//        }
    });
   ...
   // logMessage(handPosQueue)
    ...
    if(released == true){
        ...
        */
        ...
        script.motion.api.reset();
        var average_impulse = getAverageImpulse(handPosQueue);        
        average_impulse = vecMulti(average_impulse, 500);
        //var pos = new vec3(6.9, 8.2, 9.2)
        print ("adding impulse!!" + average_impulse)
        script.ball.getTransform().setWorldPosition(pos)
//        script.motion.api.ballReleased(pos);
//        script.motion.api.addImpulse(average_impulse);
        script.ball.getComponent("Physics.BodyComponent").addForce(average_impulse, Physics.ForceMode.Force);
